...
level3_energy <- data %>%
  ...
  mutate(percent=round(value/sum(value)*100,0)) %>% 
  ungroup() %>% 
  mutate(sector=if_else(percent<6,"Other",`Level 3`,)) %>% 
...
geom_line(data = level3_energy, mapping = aes(x = Year, y = value/10^6, group = interaction(sector)), show.legend = F,position = position_stack(),color="grey") +
 ...
 geom_text(mapping=aes(fill = sector, label = ifelse(percent > 2, paste0(percent, "%"), "")), position = position_stack(vjust = 0.5), size = 3, color = "black")+
  theme_void()+ 
  guides(colour = "none", fill = "none")+ scale_fill_brewer(type = "qual")
