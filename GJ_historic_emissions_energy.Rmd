...
# data <- read_excel("~/CEEW/GHG Platform/Economy-wide/GHGPI-Economy-wide-Estimates-2005-to-2018.xlsx",sheet = "Economywide estimation") %>% 
  data <- read_excel("C:/Users/Rishabh/Documents/GitHub/GJ_historical_emissions_energy/data/GHG_Gujrat-Final-Energy-2005-to-2020.xlsx", 
    sheet = "consolidated") %>% 
  # select(-27) %>% 
...
data_ippu <- read_excel("C:/Users/Rishabh/Documents/GitHub/GJ_historical_emissions_energy/data/IPPU Emissions-Revised_Final 04-02-24_20240402.xlsx", 
...
data_waste_afolu <- read_excel("C:/Users/Rishabh/Documents/GitHub/GJ_historical_emissions_energy/data/GHGPI-Economy-wide-Estimates-2005-to-2018.xlsx",     sheet = "Economywide estimation") %>% 
  filter(Gas %in% "CO2e (t) GWP-AR6") %>%
  filter(State %in% "Gujarat") %>% 
...
d <- ggplot() +
  geom_col(fuel_wise,mapping = aes(x = Year, y = value/10^6, fill = fuels))+
   geom_text(fuel_wise,mapping=aes(x = Year, y = value/10^6, fill = fuels, label = ifelse(percent > 2, paste0(percent, "%"), "")), position = position_stack(vjust = 0.5), size = 2, color = "black")+# Adjust the position and size of the text
   ggthemes::theme_pander() +
...
geom_line(data = level3_energy, mapping = aes(x = Year, y = value/10^6, group = interaction(sector)), show.legend = F,position = position_stack(),color="grey") +
  geom_point(data = level3_energy, mapping = aes(x = Year, y = value/10^6, group = interaction(sector)), show.legend = F,position = position_stack(),color="grey")+ 
   geom_text(data = level3_energy_tot, aes(x = Year, y = total/10^6, label = paste0(round(total/10^6, 0))), vjust = -0.5, color = "black", size = 2.5)+
  ggthemes::theme_pander() +
  labs(x = "", y = "Emissions (MtCO2e)",title = "Energy sector emissions")+
 ...
 b <- ggplot(data = level3_energy %>% filter(Year==2018),mapping = aes(x = Year, y = value/10^6))+
  geom_col(aes(fill=sector))+
 geom_text(mapping=aes(fill = sector, label = ifelse(percent > 2, paste0(percent, "%"), "")), position = position_stack(vjust = 0.5), size = 2.5, color = "white")+
  theme_void()+ 
  guides(colour = "none", fill = "none")+ scale_fill_viridis_d(option = "viridis")
...
ggplot() +
  ...
  geom_text(level4_industry, mapping = aes(x = Year, y = value/10^6, fill = sector, label = ifelse(percent > 2, paste0(percent, "%"), "")),
            position = position_stack(vjust = 0.5), size = 3, color = "black")+ # Adjust the position and size of the text
...
geom_text(power,mapping=aes(x = Year, y = value/1000, fill=Parameter, label = ifelse(percent > 2, paste0(round(percent,0), "%"), "")),
  ...
  geom_text(data = power_total, aes(x = Year, y = total_value/1000, label = round(total_value/1000,0)),
            vjust = -0.5, size = 2, color = "black")+  # Add total values on top of bars
 ggthemes::theme_pander() +
  labs(x = "", y = "Electricity Generation (BU)",caption = "Source: Niti Aayog")+
  scale_fill_tableau()+
...
agri2 <- read_excel("data/GROSS STATE VALUE ADDED BY ECONOMIC ACTIVITY - AGRICULTURE.XLSX",     sheet = "T_32(iv)", skip = 5,na = c("-","-*")) %>% slice(1:33) %>% mutate(`...1`=gsub("\\*","",`...1`))
...
geom_text(niti,mapping = aes(x = Year, y = value/100000,fill=Sector,label = ifelse(percent > 2, paste0(round(percent,0), "%"), "")),
