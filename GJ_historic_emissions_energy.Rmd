...
writexl::write_xlsx(data,"compiled_data_emissions.xlsx")
...
d <- ggplot() +
  geom_col(fuel_wise,mapping = aes(x = Year, y = value/10^6, fill = fuels))+
   geom_text(fuel_wise,mapping=aes(x = Year, y = value/10^6, fill = fuels, label = ifelse(percent > 2, paste0(percent, "%"), "")), position = position_stack(vjust = 0.5), size =2.2, color = "white")+# Adjust the position and size of the text
   ggthemes::theme_pander() +
